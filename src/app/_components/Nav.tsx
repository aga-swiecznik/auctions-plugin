...
import { Add, List, MoreHoriz, Person } from "@mui/icons-material";
    ...
    <Paper sx={{ position: 'fixed', bottom: 0, left: 0, right: 0, display: { md: 'none' }, zIndex: 10 }} elevation={3}>
      { sessionData ? 
        ...
        <BottomNavigation
          ...
          onChange={(event, value) => {
            setValue(value);
            if(value === "list") router.push('/api/auth/signin');
          }}
        >
          <BottomNavigationAction label="Zaloguj się" value="list" icon={<Person />} />
        </BottomNavigation>
      }
    </Paper>
  </>
